...
class DiskCache:
    ...
    def _enforce_size_limit(self) -> None:
        ...
        if total_size > self.size_limit:
            ...
            items = sorted(self._index.items(), key=lambda x: x[1]["created"])
...
class CacheKeyGenerator:
    ...
    @staticmethod
    def generate(
        prefix: str, params: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def generate_data_key(
        source: str, dataset: str, filters: Optional[Dict] = None, version: Optional[str] = None
    ) -> str:
    ...
    @staticmethod
    def generate_chart_key(chart_type: str, data_key: str, options: Optional[Dict] = None) -> str:
        """Generate cache key for charts."""
        params = {"type": chart_type, "data": data_key, "options": options or {}}
...
def cache_result(prefix: str = "result", ttl: int = 300, key_params: Optional[List[str]] = None):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            cache_key = CacheKeyGenerator.generate(f"{prefix}:{func.__name__}", params)
