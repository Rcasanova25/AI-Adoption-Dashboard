# performance/chart_optimization_simple.py - Simplified Chart Rendering Performance System
...
class DataDownsampler:
    ...
    def largest_triangle_three_buckets(data: pd.DataFrame, 
        ...
        if pd.api.types.is_datetime64_any_dtype(data_sorted[x_col]):
            x_values = data_sorted[x_col].astype(np.int64).values
        else:
...
class ChartOptimizer:
    ...
    def optimize_layout(self, fig: go.Figure) -> go.Figure:
        ...
        data_size = 0
        if has_data and hasattr(fig.data[0], 'x') and fig.data[0].x is not None:
            data_size = len(fig.data[0].x)
        ...
        optimizations = {
            ...
            'hovermode': 'closest' if has_data and data_size < 1000 else False,
    ...
    def enable_webgl(self, fig: go.Figure) -> go.Figure:
        ...
        for i, trace in enumerate(fig.data):
            if isinstance(trace, go.Scatter) and hasattr(trace, 'x') and trace.x is not None and len(trace.x) > 1000:
