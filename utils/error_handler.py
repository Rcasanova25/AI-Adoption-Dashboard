...
class ErrorHandler:
    ...
    def _show_recovery_action(self, action: str) -> None:
        ...
        actions = {
            ...
            "change_view": (
                "📊 Try Another View",
                "Select a different analysis view from the sidebar",
            ),
...
def handle_errors(
    ...
    if fallback_return is not None:
        raise ValueError(
            "CLAUDE.md compliance: fallback_return must be None. Returning sample/hardcoded data is not allowed."
        )
        ...
        def wrapper(*args, **kwargs) -> T:
            try:
                ...
                if result not in (None,):
                    raise ValueError(
                        "CLAUDE.md compliance: fallback must not return sample/hardcoded data. Only None is allowed."
                    )
                return result
