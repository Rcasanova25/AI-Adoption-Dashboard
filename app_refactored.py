...
def load_data() -> DashboardData:
    ...
    # Historical trends
    consolidated_data['historical_data'] = data_sources['ai_index'].get('adoption_trends', pd.DataFrame())
    
    # Sector data
    consolidated_data['sector_2018'] = data_sources['ai_index'].get('sector_adoption', pd.DataFrame())
    ...
    # Geographic data
    consolidated_data['geographic'] = data_sources['ai_index'].get('geographic_adoption', pd.DataFrame())
    consolidated_data['state_data'] = data_sources['ai_index'].get('state_data', pd.DataFrame())
    
    # Financial data
    consolidated_data['financial_impact'] = data_sources['mckinsey'].get('financial_impact', pd.DataFrame())
    consolidated_data['ai_investment_data'] = data_sources['ai_index'].get('investment_trends', pd.DataFrame())
...
def main():
    ...
    # Sidebar controls
    with st.sidebar:
        st.markdown('<aside role="complementary" aria-label="Dashboard controls">', unsafe_allow_html=True)
        ...
        theme = st.selectbox(
            ...
            index=['default', 'executive', 'dark', 'accessible'].index(st.session_state.selected_theme)
        )
