...
data_year = st.sidebar.selectbox(
    "Select Data Year", ["2018 (Early AI)", "2025 (GenAI Era)"], index=1
)
...
if view_type == "Historical Trends":
    year_range = st.sidebar.slider(
        "Select Year Range", min_value=2017, max_value=2025, value=(2017, 2025), step=1
    )
...
export_format = st.sidebar.selectbox(
    "Export Format", ["CSV Data", "PNG Image", "PDF Report (Beta)"]
)
...
if view_type == "Historical Trends":
    ...
    if "compare_mode" in locals() and compare_mode:
        ...
        if (
            not isinstance(year1_df, pd.DataFrame)
            or year1_df.empty
            or not isinstance(year2_df, pd.DataFrame)
            or year2_df.empty
        ):
        ...
        with col3:
            ...
            ai_cagr = (
                ((year2_data["ai_use"] / year1_data["ai_use"]) ** (1 / years_diff) - 1) * 100
                if year1_data["ai_use"] > 0
                else 0
            )
            st.metric(
                "AI CAGR",
        ...
        with col4:
            if year1_data["genai_use"] > 0:
                genai_cagr = (
                    (year2_data["genai_use"] / year1_data["genai_use"]) ** (1 / years_diff) - 1
                ) * 100
                st.metric(
                    "GenAI CAGR",
                    f"{genai_cagr:.1f}%",
        ...
        **📈 Key Changes from {year1} to {year2}:**
        ...
        visible_milestones = [
            m
            for m in authoritative_milestones
            if isinstance(m["year"], (int, float)) and year_range[0] <= m["year"] <= year_range[1]
        ]
        ...
        if (
            isinstance(filtered_data, pd.DataFrame)
            and "year" in filtered_data.columns
            and "ai_use" in filtered_data.columns
        ):
            fig.add_trace(
                go.Scatter(
        ...
        if (
            isinstance(filtered_data, pd.DataFrame)
            and "year" in filtered_data.columns
            and "genai_use" in filtered_data.columns
        ):
            fig.add_trace(
                go.Scatter(
                    x=filtered_data["year"],
        ...
        for milestone in visible_milestones:
            if milestone["year"] in filtered_data["year"].values:
                height = (
                    filtered_data[filtered_data["year"] == milestone["year"]]["ai_use"].iloc[0] + 5
                )
            else:
                continue
    ...
    export_data = (
        filtered_data if not ("compare_mode" in locals() and compare_mode) else historical_data
    )
        ...
        source_credibility = pd.DataFrame(
            {
                "Source Type": [
                    "Government (NSF, NIST)",
                    "Academic (Stanford, MIT, Nature)",
                    "Industry (OpenAI, GitHub)",
    ...
    **📊 Understanding the Data:** - The percentages below show the **proportion of companies reporting financial benefits** from AI
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📈 Overall Trends", "🌍 Geographic Distribution", "🚀 GenAI Focus", "📊 Comparative Analysis"]
    )
    ...
    with tab3:
        ...
        fig.add_trace(
            go.Bar(
                ...
                text=[
                    f"${x:.1f}B<br>{g}"
                    for x, g in zip(genai_data["investment"], genai_data["growth"])
                ],
                textposition="outside",
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                type="category", categoryorder="array", categoryarray=["2022", "2023", "2024"]
            ),
        )
    ...
    **Strategic Insights:**
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["What Are Tokens?", "Token Pricing", "Usage Patterns", "Optimization", "Economic Impact"]
    )
        ...
        with col1:
            ...
            fig2.add_trace(
                go.Bar(
                    ...
                    y=token_economics.sort_values("cost_per_million_input")[
    ...
    with tab4:
        ...
        fig.add_annotation(
            x=30,
            y=90,
            text="High Priority<br>Low Investment",
            showarrow=False,
            font=dict(color="red"),
        )
        fig.add_annotation(
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Training Emissions", "Energy Trends", "Mitigation Strategies", "Sustainability Metrics"]
    )
    ...
    with tab3:
        ...
        fig.add_annotation(
            x=85, y=85, text="Target Zone", showarrow=False, font=dict(color="green")
        )
...
# Continue with all remaining view implementations...
    ...
    **📈 Key Insights:**
    ...
    # Quadrant labels
    fig.add_annotation(
        x=25, y=75, text="High Risk<br>Low Adoption", showarrow=False, font=dict(color="gray")
    )
    fig.add_annotation(
        x=75, y=75, text="High Risk<br>High Adoption", showarrow=False, font=dict(color="gray")
    )
    fig.add_annotation(
        x=25, y=25, text="Low Risk<br>Low Adoption", showarrow=False, font=dict(color="gray")
    )
    fig.add_annotation(
        x=75, y=25, text="Low Risk<br>High Adoption", showarrow=False, font=dict(color="gray")
    )
    ...
    with geo_tabs[2]:
        ...
        rankings_display = state_scorecard[display_cols].sort_values(
    ...
    **Recommended Approach:** Start with regulatory clarity and tax incentives for immediate impact while building 
    ...
    with tab3:
        ...
        for _, sector in top_sectors.iterrows():
            st.write(
                f"• **{sector['sector']}:** {sector['avg_roi']}x ROI, {sector['adoption_rate']}% adoption"
            )
    ...
    with tab4:
        ...
        with col3:
            st.metric(
                "Net Benefit",
                f"${net_benefit:,.0f}",
                delta=f"{(net_benefit/investment_amount)*100:.0f}%",
            )
        with col4:
            st.metric(
                "Payback Period", f"{payback_months} months", help="Time to recover investment"
            )
