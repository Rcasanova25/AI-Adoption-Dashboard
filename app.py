...
def get_persona_icon(persona):
    ...
    return icons.get(persona, "👤")\n\ndef render_strategic_command_center(persona, dynamic_metrics, historical_data):\n    \"\"\"Render the Strategic Command Center for each persona\"\"\"\n    \n    if persona == \"Executive\":\n        render_executive_command_center(dynamic_metrics, historical_data)\n    elif persona == \"Policymaker\":\n        render_policymaker_command_center(dynamic_metrics, historical_data)\n    elif persona == \"Researcher\":\n        render_researcher_command_center(dynamic_metrics, historical_data)\n    else:\n        # General user gets overview of all command centers\n        render_general_overview(dynamic_metrics, historical_data)\n\ndef render_executive_command_center(dynamic_metrics, historical_data):\n    \"\"\"Executive Strategic Command Center - High-level business insights\"\"\"\n    \n    # Custom header for executive dashboard\n    st.markdown(\"\"\"\n    <div style=\"background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); \n                padding: 2rem; border-radius: 15px; margin: 2rem 0; color: white;\">\n        <h1 style=\"margin: 0; color: white; text-align: center;\">💼 Executive Strategic Command Center</h1>\n        <p style=\"margin: 0.5rem 0 0 0; text-align: center; opacity: 0.9;\">Strategic insights for C-level decision making</p>\n    </div>\n    \"\"\", unsafe_allow_html=True)\n    \n    # Executive KPI Dashboard\n    st.subheader(\"📊 Key Performance Indicators\")\n    \n    # Top-level metrics in cards\n    col1, col2, col3, col4 = st.columns(4)\n    \n    with col1:\n        st.metric(\n            label=\"🎯 Market Position\", \n            value=\"Top 25%\",\n            delta=\"+5% vs competition\",\n            help=\"Your position in AI adoption relative to industry\"\n        )\n    \n    with col2:\n        st.metric(\n            label=\"💰 ROI Potential\", \n            value=\"4.2x\",\n            delta=\"+15% vs baseline\",\n            help=\"Expected return on AI investment\"\n        )\n    \n    with col3:\n        st.metric(\n            label=\"⚡ Implementation Speed\", \n            value=\"8.5/10\",\n            delta=\"Fast Track\",\n            help=\"Readiness for AI implementation\"\n        )\n    \n    with col4:\n        st.metric(\n            label=\"🎯 Competitive Advantage\", \n            value=\"High\",\n            delta=\"Strategic Priority\",\n            help=\"AI's impact on competitive positioning\"\n        )\n    \n    # Strategic Insights Dashboard\n    col1, col2 = st.columns([2, 1])\n    \n    with col1:\n        st.subheader(\"📈 Market Intelligence Summary\")\n        \n        if historical_data is not None and not historical_data.empty:\n            # Create executive summary chart\n            fig = create_market_intelligence_chart(historical_data)\n            if fig:\n                st.plotly_chart(fig, use_container_width=True)\n        \n        # Strategic recommendations\n        st.subheader(\"🎯 Strategic Recommendations\")\n        st.markdown(\"\"\"\n        **Immediate Actions (0-90 days):**\n        - Focus on high-ROI use cases in Marketing & Sales\n        - Establish AI governance framework\n        - Begin pilot program with 2-3 key departments\n        \n        **Medium-term Strategy (3-12 months):**\n        - Scale successful pilots across organization\n        - Invest in employee AI training programs\n        - Develop competitive AI capabilities\n        \n        **Long-term Vision (1-3 years):**\n        - Achieve industry-leading AI adoption (>75%)\n        - Establish AI-driven business model innovations\n        - Lead market transformation through AI\n        \"\"\")\n    \n    with col2:\n        st.subheader(\"⚠️ Risk Assessment\")\n        \n        # Risk dashboard\n        risk_data = {\n            \"Implementation Risk\": {\"level\": \"Medium\", \"color\": \"orange\"},\n            \"Competitive Risk\": {\"level\": \"High\", \"color\": \"red\"},\n            \"Technology Risk\": {\"level\": \"Low\", \"color\": \"green\"},\n            \"Talent Risk\": {\"level\": \"High\", \"color\": \"red\"}\n        }\n        \n        for risk, info in risk_data.items():\n            color = info[\"color\"]\n            level = info[\"level\"]\n            st.markdown(f\"**{risk}:** <span style='color: {color}; font-weight: bold;'>{level}</span>\", unsafe_allow_html=True)\n        \n        st.subheader(\"💡 Quick Actions\")\n        \n        if st.button(\"📊 View Investment Case\", type=\"primary\"):\n            st.session_state.strategic_view_selection = \"💰 Investment Decision Engine\"\n            st.rerun()\n        \n        if st.button(\"🎯 Competitive Analysis\"):\n            st.session_state.strategic_view_selection = \"🎯 Competitive Position Assessor\"\n            st.rerun()\n        \n        if st.button(\"📈 Market Intelligence\"):\n            st.session_state.strategic_view_selection = \"📊 Market Intelligence\"\n            st.rerun()\n        \n        if st.button(\"📋 Action Planning\"):\n            st.session_state.strategic_view_selection = \"📋 Action Planning\"\n            st.rerun()\n\ndef render_policymaker_command_center(dynamic_metrics, historical_data):\n    \"\"\"Policymaker Strategic Command Center - Policy insights\"\"\"\n    \n    st.markdown(\"\"\"\n    <div style=\"background: linear-gradient(135deg, #ff7f0e 0%, #d4641a 100%); \n                padding: 2rem; border-radius: 15px; margin: 2rem 0; color: white;\">\n        <h1 style=\"margin: 0; color: white; text-align: center;\">🏛️ Policymaker Command Center</h1>\n        <p style=\"margin: 0.5rem 0 0 0; text-align: center; opacity: 0.9;\">Policy insights for government and regulatory leaders</p>\n    </div>\n    \"\"\", unsafe_allow_html=True)\n    \n    st.info(\"🚧 Policymaker Command Center coming in Phase 3\")\n    st.markdown(\"Strategic policy dashboard will include:\")\n    st.markdown(\"- Geographic impact analysis\")\n    st.markdown(\"- Labor market assessments\")\n    st.markdown(\"- Regulatory compliance tracking\")\n    st.markdown(\"- International comparisons\")\n\ndef render_researcher_command_center(dynamic_metrics, historical_data):\n    \"\"\"Researcher Strategic Command Center - Research insights\"\"\"\n    \n    st.markdown(\"\"\"\n    <div style=\"background: linear-gradient(135deg, #2ca02c 0%, #256629 100%); \n                padding: 2rem; border-radius: 15px; margin: 2rem 0; color: white;\">\n        <h1 style=\"margin: 0; color: white; text-align: center;\">🔬 Research Analytics Hub</h1>\n        <p style=\"margin: 0.5rem 0 0 0; text-align: center; opacity: 0.9;\">Deep analytical insights for researchers and analysts</p>\n    </div>\n    \"\"\", unsafe_allow_html=True)\n    \n    st.info(\"🚧 Research Analytics Hub coming in Phase 3\")\n    st.markdown(\"Research command center will include:\")\n    st.markdown(\"- Methodology overviews\")\n    st.markdown(\"- Data quality indicators\")\n    st.markdown(\"- Research gap analysis\")\n    st.markdown(\"- Citation and source tracking\")\n\ndef render_general_overview(dynamic_metrics, historical_data):\n    \"\"\"General user overview of all command centers\"\"\"\n    \n    st.markdown(\"\"\"\n    <div style=\"background: linear-gradient(135deg, #9467bd 0%, #6b46a3 100%); \n                padding: 2rem; border-radius: 15px; margin: 2rem 0; color: white;\">\n        <h1 style=\"margin: 0; color: white; text-align: center;\">👤 General Explorer Dashboard</h1>\n        <p style=\"margin: 0.5rem 0 0 0; text-align: center; opacity: 0.9;\">Comprehensive access to all strategic insights</p>\n    </div>\n    \"\"\", unsafe_allow_html=True)\n    \n    st.info(\"🚧 General Explorer Dashboard coming in Phase 3\")\n    st.markdown(\"General dashboard will provide access to all persona-specific insights:\")\n    st.markdown(\"- Executive strategic summaries\")\n    st.markdown(\"- Policy impact assessments\")\n    st.markdown(\"- Research methodology insights\")\n    st.markdown(\"- Complete analysis library\")\n\ndef render_breadcrumb_navigation(persona, current_view, is_detailed):\n    \"\"\"Render breadcrumb navigation for enhanced UX\"\"\"\n    \n    # Build breadcrumb path\n    breadcrumbs = []\n    \n    # Start with persona\n    persona_icon = get_persona_icon(persona)\n    breadcrumbs.append(f\"{persona_icon} {persona}\")\n    \n    # Add dashboard mode\n    if current_view == \"STRATEGIC_DASHBOARD\":\n        breadcrumbs.append(\"🎯 Strategic Command Center\")\n    elif is_detailed:\n        breadcrumbs.append(\"📊 Detailed Analytics\")\n    else:\n        breadcrumbs.append(\"🎯 Executive Views\")\n    \n    # Add current view if not strategic dashboard\n    if current_view != \"STRATEGIC_DASHBOARD\":\n        breadcrumbs.append(current_view)\n    \n    # Render breadcrumbs\n    breadcrumb_html = \" → \".join(breadcrumbs)\n    st.markdown(f\"\"\"\n    <div style=\"background: rgba(0,0,0,0.05); padding: 0.5rem 1rem; border-radius: 5px; margin-bottom: 1rem;\">\n        <small style=\"color: #666;\">📍 Navigation: {breadcrumb_html}</small>\n    </div>\n    \"\"\", unsafe_allow_html=True)\n\ndef render_related_insights(current_view, persona):\n    \"\"\"Show related insights and cross-references\"\"\"\n    \n    # Define related views for each analysis\n    related_views_map = {\n        \"🎯 Competitive Position Assessor\": [\"💰 Investment Decision Engine\", \"Industry Analysis\", \"🏭 Firm Size Analysis\"],\n        \"💰 Investment Decision Engine\": [\"ROI Analysis\", \"Financial Impact\", \"AI Cost Trends\"],\n        \"📊 Market Intelligence\": [\"Historical Trends\", \"Industry Analysis\", \"Geographic Distribution\"],\n        \"Industry Analysis\": [\"🎯 Competitive Position Assessor\", \"🏭 Firm Size Analysis\", \"📊 Market Intelligence\"],\n        \"ROI Analysis\": [\"💰 Investment Decision Engine\", \"Financial Impact\", \"AI Cost Trends\"],\n        \"🏭 Firm Size Analysis\": [\"Industry Analysis\", \"🎯 Competitive Position Assessor\", \"💰 Investment Decision Engine\"],\n        \"Geographic Distribution\": [\"📊 Market Intelligence\", \"Regional Growth\", \"🌍 OECD 2025 Findings\"],\n        \"🎓 Skill Gap Analysis\": [\"🚧 Barriers & Support\", \"Labor Impact\", \"⚖️ AI Governance\"],\n        \"⚖️ AI Governance\": [\"🎓 Skill Gap Analysis\", \"🚧 Barriers & Support\", \"⚖️ Regulatory Risk Radar\"]\n    }\n    \n    if current_view in related_views_map:\n        related_views = related_views_map[current_view]\n        \n        # Filter based on persona access\n        if persona in persona_views:\n            accessible_views = persona_views[persona].get(\"detailed\", []) + persona_views[persona].get(\"strategic\", [])\n            related_views = [view for view in related_views if view in accessible_views]\n        \n        if related_views:\n            st.sidebar.markdown(\"---\")\n            st.sidebar.markdown(\"### 🔗 Related Insights\")\n            \n            for view in related_views[:3]:  # Show top 3 related views\n                if st.sidebar.button(f\"➤ {view}\", key=f\"related_{view}\"):\n                    st.session_state.strategic_view_selection = view\n                    st.rerun()\n\ndef enhance_persona_navigation():\n    \"\"\"Enhanced navigation with persona context\"\"\"\n    \n    current_persona = st.session_state.get('selected_persona', 'General')\n    \n    # Add persona context to sidebar\n    st.sidebar.markdown(\"---\")\n    st.sidebar.markdown(\"### 🎯 Quick Actions\")\n    \n    # Persona-specific quick actions\n    if current_persona == \"Executive\":\n        if st.sidebar.button(\"📊 Strategic Dashboard\", type=\"primary\"):\n            st.session_state.strategic_view_selection = \"STRATEGIC_DASHBOARD\"\n            st.rerun()\n        \n        if st.sidebar.button(\"💰 Investment Case\"):\n            st.session_state.strategic_view_selection = \"💰 Investment Decision Engine\"\n            st.rerun()\n            \n        if st.sidebar.button(\"🎯 Market Position\"):\n            st.session_state.strategic_view_selection = \"🎯 Competitive Position Assessor\"\n            st.rerun()\n    \n    elif current_persona == \"Policymaker\":\n        if st.sidebar.button(\"🏛️ Policy Dashboard\", type=\"primary\"):\n            st.session_state.strategic_view_selection = \"STRATEGIC_DASHBOARD\"\n            st.rerun()\n            \n        if st.sidebar.button(\"🌍 Geographic Analysis\"):\n            st.session_state.strategic_view_selection = \"Geographic Distribution\"\n            st.rerun()\n            \n        if st.sidebar.button(\"⚖️ Governance Framework\"):\n            st.session_state.strategic_view_selection = \"⚖️ AI Governance\"\n            st.rerun()\n    \n    elif current_persona == \"Researcher\":\n        if st.sidebar.button(\"🔬 Research Hub\", type=\"primary\"):\n            st.session_state.strategic_view_selection = \"STRATEGIC_DASHBOARD\"\n            st.rerun()\n            \n        if st.sidebar.button(\"📊 Historical Analysis\"):\n            st.session_state.strategic_view_selection = \"Historical Trends\"\n            st.rerun()\n            \n        if st.sidebar.button(\"🎓 Skills & Barriers\"):\n            st.session_state.strategic_view_selection = \"🎓 Skill Gap Analysis\"\n            st.rerun()\n\ndef determine_navigation_mode(dynamic_metrics):
    """Determine which navigation system to use - FIXED"""
    
    # DISABLED OLD NAVIGATION - Using persona-driven system instead\n    current_persona = st.session_state.get('selected_persona', 'General')\n    if current_persona in persona_views:\n        return create_persona_navigation(current_persona, dynamic_metrics)\n    \n    # LEGACY FALLBACK - Let users choose their experience
...
if st.session_state.first_visit:
    ...
    # Available personas for MVP testing (Option C: Hide incomplete personas)
    available_personas = ["Executive", "General"]  # Policymaker & Researcher coming soon
    
    # Enhanced persona cards - Only show completed personas
    col1, col2 = st.columns(2)
    ...
    # Coming Soon section for incomplete personas
    st.markdown("---")
    st.markdown("""
...
# Persona switcher for power users (optional) - Limited to available personas
current_persona = st.session_state.get('selected_persona', 'General')
if st.sidebar.checkbox("🔄 Switch Persona", value=False, help="Switch between available persona views"):
    # Only show fully implemented personas
    available_personas_testing = ["Executive", "General"]  # MVP testing personas only
    
    if current_persona not in available_personas_testing:
        # Handle legacy or unavailable personas
        if current_persona in ["Business Leader", "Policymaker", "Researcher"]:
            current_persona = "Executive"  # Default to working persona
        else:
            current_persona = "General"
    
    persona = st.sidebar.selectbox(
        "Select Your Role",
        available_personas_testing,
        index=available_personas_testing.index(current_persona),
        help="Additional personas coming after testing phase"
    )
    ...
    # Show notice about coming personas
    if current_persona in ["Executive", "General"]:
        st.sidebar.info("🚧 **Policymaker & Researcher** personas coming after comprehensive testing")\n\n# Legacy persona selection (DISABLED for MVP testing)
# persona = st.sidebar.selectbox(
#     "Select Your Role",
...
# ENHANCED ROUTING LOGIC - Persona-driven with Strategic Command Centers\nif current_view == \"STRATEGIC_DASHBOARD\":\n    # Handle Strategic Command Center for each persona\n    render_strategic_command_center(st.session_state.get('selected_persona', 'General'), dynamic_metrics, historical_data)\n\n# IMPROVED ROUTING LOGIC
elif not is_detailed:
    # EXECUTIVE VIEWS - High-level strategic dashboard views
    # Handle strategic view selections from command center buttons\n    if st.session_state.get('strategic_view_selection'):\n        current_view = st.session_state.strategic_view_selection\n        st.session_state.strategic_view_selection = None  # Clear after use\n        st.rerun()\n    \n    if current_view == "🚀 Strategic Brief":
        executive_strategic_brief(dynamic_metrics, historical_data)\n\n# === FIXED ROUTING SECTION - IGNORE ABOVE MALFORMED CODE ===\n# Handle STRATEGIC_DASHBOARD properly\nif current_view == \"STRATEGIC_DASHBOARD\":\n    render_strategic_command_center(st.session_state.get('selected_persona', 'General'), dynamic_metrics, historical_data)\n    st.stop()  # Prevent further processing\n\n# Handle strategic view button selections\nif st.session_state.get('strategic_view_selection'):\n    current_view = st.session_state.strategic_view_selection\n    st.session_state.strategic_view_selection = None\n    st.rerun()
    ...
    # ADD ALL OTHER DETAILED VIEWS HERE...
        ...
        if geographic is not None and safe_data_check(geographic, "Geographic Distribution"):
            ...
            if 'ai_research_centers' not in enhanced_geographic.columns:
                enhanced_geographic['ai_research_centers'] = [15, 8, 5, 3, 12, 4, 2, 1]
            if 'federal_ai_funding_millions' not in enhanced_geographic.columns:
                enhanced_geographic['federal_ai_funding_millions'] = [2100, 650, 350, 165, 890, 420, 285, 95]
            if 'ai_startups' not in enhanced_geographic.columns:
                enhanced_geographic['ai_startups'] = [850, 450, 145, 85, 325, 185, 125, 55]
            if 'venture_capital_millions' not in enhanced_geographic.columns:
                enhanced_geographic['venture_capital_millions'] = [15800, 8500, 2850, 650, 4200, 3850, 1250, 385]
            if 'nsf_ai_institutes' not in enhanced_geographic.columns:
                enhanced_geographic['nsf_ai_institutes'] = [2, 2, 1, 1, 3, 1, 1, 0]
            ...
            with geo_tabs[0]:
                ...
                fig.add_trace(go.Scattergeo(
                    ...
                    customdata=enhanced_geographic[[
                        'rate', 'federal_ai_funding_millions', 'ai_research_centers', 
            ...
            with geo_tabs[1]:
                ...
                top_cities = enhanced_geographic.nlargest(10, 'rate')
                ...
                fig2.add_trace(go.Bar(
                    ...
                    y=top_cities['rate'],
                    marker_color='#1f77b4',
                    text=top_cities['rate'].round(1).astype(str) + '%',
                    textposition='outside'
                ))
                ...
                col1, col2, col3 = st.columns(3)
                with col1:
                    avg_adoption = enhanced_geographic['rate'].mean()
                    st.metric("Average Adoption", f"{avg_adoption:.1f}%")
                
                with col2:
                    total_startups = enhanced_geographic['ai_startups'].sum()
                    st.metric("Total AI Startups", f"{total_startups:,}")
                
                with col3:
                    total_funding = enhanced_geographic['federal_ai_funding_millions'].sum()
                    st.metric("Total Federal Funding", f"${total_funding:,.0f}M")
            
            with geo_tabs[2]:
                st.markdown("### Research Infrastructure Overview")
                ...
                with col1:
                    ...
                    fig3.add_trace(go.Scatter(
                        x=enhanced_geographic['ai_research_centers'],
                        y=enhanced_geographic['rate'],
                        mode='markers+text',
                        text=enhanced_geographic['city'],
                        textposition='top center',
                        marker=dict(
                            size=enhanced_geographic['major_universities'] * 2,
                            color=enhanced_geographic['nsf_ai_institutes'],
                            colorscale='Viridis',
                            showscale=True,
                            colorbar=dict(title="NSF Institutes")
                        )
                    ))
                    fig3.update_layout(
                        title="Research Centers vs AI Adoption",
                        xaxis_title="AI Research Centers",
                        yaxis_title="AI Adoption Rate (%)",
                        height=400
                    )
                ...
                with col2:
                    st.markdown("### 🏛️ Research Infrastructure")
                    ...
                    total_universities = enhanced_geographic['major_universities'].sum()
                    
                    st.metric("AI Research Centers", total_research_centers)
                    st.metric("NSF AI Institutes", total_nsf_institutes)
                    st.metric("Major Universities", total_universities)
                    
                    st.markdown("**Key Research Hubs:**")
                    research_hubs = enhanced_geographic[enhanced_geographic['ai_research_centers'] >= 10]
                    for _, hub in research_hubs.iterrows():
                        st.markdown(f"• **{hub['city']}**: {hub['ai_research_centers']} centers")
...
# Add related insights for current view\nrender_related_insights(current_view, current_persona)\n\n# PERFORMANCE MONITORING & OPTIMIZATION SECTION
st.sidebar.markdown("---")
