...
def test_payback_calculations():
    ...
    for industry in industries:
        ...
        print(f"\n{industry}:")
        print(
            f"  - Payback period: {result['payback_months']:.0f} months ({result['payback_years']:.1f} years)"
        )
...
def compare_old_vs_new():
    ...
    print(
        f"  Difference: {(new_result['cumulative_productivity_gain'] / old_productivity_gain - 1) * 100:.0f}% higher"
    )
    ...
    new_market = models.calculate_market_value_impact(
        revenue=revenue, industry="Technology", ai_adoption_level=50, competitive_position="Average"
    )
    ...
    print(
        f"  Difference: {(new_market['market_value_increase'] / old_market_impact - 1) * 100:.0f}% higher"
    )
