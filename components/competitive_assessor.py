...
class CompetitivePositionAssessor:
    ...
    def __init__(self, sector_data: pd.DataFrame, firm_size_data: pd.DataFrame, 
                 adoption_trends: pd.DataFrame, investment_data: pd.DataFrame):
        ...
        **Welcome to the Economics of AI Dashboard** - Your strategic companion for navigating 
        ...
        with col2:
            company_size = st.selectbox(
                "Company Size",
                options=["Small (<50)", "Medium (50-999)", "Large (1000-4999)", "Enterprise (5000+)"],
        ...
        if st.button("🚀 Generate Assessment", type="primary", use_container_width=True):
            self._generate_assessment(
                industry, company_size, current_adoption,
                annual_revenue, ai_investment, growth_target, risk_tolerance
            )
    
    def _generate_assessment(self, industry: str, company_size: str, 
                           current_adoption: float, annual_revenue: float,
                           ai_investment: float, growth_target: float,
                           risk_tolerance: str):
        ...
        your_metrics = {
            ...
            'investment_intensity': (ai_investment / annual_revenue) * 100 if annual_revenue > 0 else 0
        }
        ...
        with tab1:
            self._display_executive_summary(
                industry, current_adoption, industry_avg_adoption,
                position_data, sector_info
            )
        
        with tab2:
            self._display_competitive_position(
                your_metrics, sector_info, position_data, peer_data
            )
        
        with tab3:
            self._display_economic_impact(
                company_size, industry, current_adoption,
                annual_revenue, ai_investment, growth_target
            )
        
        with tab4:
            self._display_what_if_scenarios(
                your_metrics, industry, company_size,
                annual_revenue, risk_tolerance
            )
        
        with tab5:
            self._display_action_plan(
                position_data, industry, company_size,
                current_adoption, growth_target
            )
    
    def _display_executive_summary(self, industry: str, current_adoption: float,
                                 industry_avg: float, position_data: Dict,
        ...
        if position_data['position'] == "Leader":
            key_points.append(f"You're ahead of {position_data['adoption_percentile']:.0f}% of peers")
            key_points.append("Well-positioned to capture AI value creation")
        else:
            key_points.append(f"You're behind {100-position_data['adoption_percentile']:.0f}% of peers")
        ...
        if position_data['position'] in ["Follower", "Laggard"]:
            recommendations.append(f"Increase AI investment by {position_data['recommended_investment_increase']:.0f}%")
        ...
        with col3:
            st.metric(
                "Risk Level",
                position_data['risk_level'],
                help="Risk of competitive displacement"
            )
    ...
    def _display_competitive_position(self, your_metrics: Dict, sector_info: pd.Series,
                                    position_data: Dict, peer_data: pd.DataFrame):
        ...
        fig = self.insights.create_competitive_position_matrix(
            your_metrics['adoption'],
            your_metrics['investment'],
            self.sector_data
        )
        ...
        with col1:
            ...
            fig_adoption.add_trace(go.Box(
                y=peer_data['adoption_rate'],
                name="Industry Peers",
                boxpoints='outliers'
            ))
        ...
        with col2:
            ...
            fig_investment.add_trace(go.Box(
                y=peer_data['investment'],
                name="Industry Peers",
                boxpoints='outliers'
            ))
    ...
    def _display_economic_impact(self, company_size: str, industry: str,
                               current_adoption: float, annual_revenue: float,
                               ai_investment: float, growth_target: float):
        ...
        fig.add_trace(go.Scatter(
            x=months, y=conservative_roi,
            name="Conservative",
            line=dict(dash='dash', color='gray')
        ))
        fig.add_trace(go.Scatter(
            x=months, y=expected_roi,
            name="Expected",
            line=dict(color='blue', width=3)
        ))
        fig.add_trace(go.Scatter(
            x=months, y=optimistic_roi,
            name="Optimistic",
        ...
        with col1:
            ...
            st.metric(
                "Payback Period",
                f"{payback_period} months",
                help="Time to recover AI investment"
            )
    ...
    def _display_what_if_scenarios(self, your_metrics: Dict, industry: str,
                                  company_size: str, annual_revenue: float,
                                  risk_tolerance: str):
        ...
        if scenario_type == "Adoption Acceleration":
            scenarios = self.insights.generate_what_if_scenarios(
                base_data, "adoption_acceleration"
            )
            ...
            fig.update_layout(
                title="Impact of Accelerating AI Adoption",
                barmode='group',
                height=400
            )
            st.plotly_chart(fig, use_container_width=True)
            ...
            scenarios = self.insights.generate_what_if_scenarios(
                base_data, "competitive_response"
            )
            
            # Radar chart for competitive scenarios
            categories = ['Market Position', 'Revenue Impact', 
                         'Talent Retention', 'Innovation Score']
            
            fig = go.Figure()
            
            for _, row in scenarios.iterrows():
                fig.add_trace(go.Scatterpolar(
                    r=[row['market_position'], row['revenue_impact'],
                       row['talent_retention'], row['innovation_score']],
                    theta=categories,
                    fill='toself',
                    name=row['scenario']
                ))
    ...
    def _display_action_plan(self, position_data: Dict, industry: str,
                           company_size: str, current_adoption: float,
                           growth_target: float):
        ...
        self.insights.display_action_plan(
            urgency, timeline, key_actions, success_metrics
        )
    ...
    def _create_peer_data(self, industry: str, company_size: str) -> pd.DataFrame:
        ...
        peer_data = pd.DataFrame({
            ...
            'investment': np.random.normal(base_investment * size_factor, 
                                         base_investment * size_factor * 0.3, n_peers).clip(0, None),
            'genai_adoption': np.random.normal(sector_info['genai_adoption'], 10, n_peers).clip(0, 100),
            'employees': np.random.normal(1000 * size_factor, 300 * size_factor, n_peers).clip(50, None)
        })
        
        return peer_data
    
    def _identify_advantages(self, your_metrics: Dict, sector_info: pd.Series,
                           peer_data: pd.DataFrame) -> List[str]:
    ...
    def _identify_gaps(self, your_metrics: Dict, sector_info: pd.Series,
                      peer_data: pd.DataFrame) -> List[str]:
    ...
    def _calculate_roi_curve(self, months: np.ndarray, investment: float,
