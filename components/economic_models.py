...
class AIEconomicModels:
    ...
    def calculate_cost_of_inaction(
        ...
        sector_gain = self.params.sector_productivity_gains.get(
            industry, 
            self.params.sector_productivity_gains['Other']
        )
        ...
        productivity_loss = self._calculate_productivity_loss(
            current_revenue, years, sector_gain
        )
        ...
        gdp_opportunity_cost = self._calculate_gdp_opportunity_cost(
            current_revenue, years
        )
        
        # 5. Talent and capability gap cost
        capability_gap_cost = self._calculate_capability_gap_cost(
            current_revenue, years
        )
    ...
    def _calculate_market_share_loss(
        ...
        competitive_risk = 1 - np.exp(-self.params.competitive_lambda * competitors_adopting_pct / 100)
    ...
    def _calculate_gdp_opportunity_cost(
        self,
        revenue: float,
        years: int
    ) -> float:
    ...
    def _calculate_capability_gap_cost(
        self,
        revenue: float,
        years: int
    ) -> float:
    ...
    def _assess_market_position_risk(
        self,
        years: int,
        competitors_adopting_pct: float
    ) -> str:
    ...
    def calculate_roi_with_real_data(
        ...
        base_roi = self.params.use_case_roi.get(
            use_case, 
            self.params.use_case_roi['Default']
        )
        ...
        for year in range(implementation_years):
            ...
            learning_efficiency = 1 - (1 - self.params.initial_efficiency) * np.exp(-self.params.learning_rate * (year + 1))
            
            # Calculate diminishing returns
            diminishing_factor = 1 - np.exp(-0.5 * (year + 1))  # Diminishing returns over time
            
            year_return = investment * adjusted_roi * adoption_rate * learning_efficiency * diminishing_factor / implementation_years
        ...
        payback_period = next(
            (i + 1 for i, cf in enumerate(cumulative_cf) if cf > 0), 
        ...
        industry_payback = self.params.industry_payback_periods.get(industry, 18) / 12  # Convert to years
    ...
    def get_confidence_intervals(
        ...
        factors = confidence_factors.get(
            metric_type,
            {'low': 0.8, 'high': 1.2}  # Default
        )
    ...
    def calculate_market_value_impact(
        ...
        network_value = self.params.network_base_value * (adoption_factor ** self.params.network_coefficient)
        
        # Calculate market value impact
        base_market_cap = revenue * pe_ratio
        ai_enhanced_market_cap = base_market_cap * growth_multiple * adoption_factor * position_mult * network_value
    ...
    def validate_calculations(self) -> Dict[str, bool]:
        ...
        validations['market_value_uses_pe_ratios'] = (
            market_result['pe_ratio'] > 0
        )
        ...
        validations['payback_includes_learning_curve'] = (
            payback_result['learning_curve_months'] > 0
        )
