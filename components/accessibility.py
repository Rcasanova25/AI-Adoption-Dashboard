...
class AccessibilityManager:
    ...
    def make_chart_accessible(
        ...
        title: str,
        description: str,
        data_table: Optional[Dict[str, List]] = None
    ) -> go.Figure:
        ...
        fig.update_traces(
            marker=dict(
                line=dict(width=2, color='#000000')  # Add black outline
            )
        )
    ...
    def create_accessible_table(
        ...
        if summary:
            st.markdown(
                f'<div class="sr-only" id="table-summary">{summary}</div>',
                unsafe_allow_html=True
            )
    ...
    def apply_accessibility_settings(self):
        ...
        if self.config.enable_high_contrast:
            st.markdown(
                '<div class="high-contrast-wrapper high-contrast">',
                unsafe_allow_html=True
            )
