...
class EnhancedCompetitiveAssessor(CompetitivePositionAssessor):
    ...
    def __init__(self, sector_data: pd.DataFrame, firm_size_data: pd.DataFrame,
        ...
        # Initialize economic models and validator
    ...
    def render(self):
        ...
        *Using real economic models based on Goldman Sachs, McKinsey, and industry data.*
        ...
        # Quick assessment section with validation
        st.markdown("---")
        ...
        with st.expander("🔧 Advanced Settings", expanded=False):
            ...
            with col1:
                ...
                # Convert to actual dollars for validation
                revenue_dollars = annual_revenue * 1_000_000
                
                ai_investment = st.number_input(
                    "Annual AI Investment ($M)",
                    min_value=0.0,
                    max_value=annual_revenue * 0.1,  # Max 10% of revenue
                    value=min(5.0, annual_revenue * 0.03),  # Default 3% of revenue
                    step=0.5,
        ...
        # Validate inputs before generating assessment
        if st.button("🚀 Generate Assessment", type="primary", use_container_width=True):
            ...
            if not is_valid:
                ...
                st.info(f"{confidence_info['icon']} **Data Confidence: {confidence_info['level']}** - {confidence_info['description']}")
                
                # Generate assessment with real calculations
                self._generate_enhanced_assessment(
                    industry, company_size, current_adoption,
                    revenue_dollars, investment_dollars, employees,
                    growth_target, risk_tolerance, confidence_scores
                )
    ...
    def _generate_enhanced_assessment(
        ...
        with tab1:
            self._display_enhanced_executive_summary(
                ...
                your_metrics, confidence_scores
            )
        
        with tab2:
            self._display_enhanced_competitive_position(
    ...
    def _display_enhanced_executive_summary(
        ...
        if position in ["Follower", "Laggard"]:
            ...
            if current_intensity < target_intensity:
                increase_pct = ((target_intensity - current_intensity) / current_intensity * 100) if current_intensity > 0 else 200
                recommendations.append(f"Increase AI investment by {increase_pct:.0f}% to reach industry standards")
        ...
        with col3:
            ...
            risk_factor = 1 - np.exp(-0.3 * competitors_adopting / 100) if current_adoption < industry_avg else 0.2
    ...
    def _display_real_economic_impact(
        ...
        if delay_years > 0:
            ...
            with st.expander("📊 Detailed Cost Breakdown"):
                ...
                fig.update_layout(
                    title="Cost of Inaction Components",
                    xaxis_title="Cost ($M)",
                    height=300
                )
    ...
    def _display_enhanced_scenarios(
        ...
        if scenario_type == "Investment Levels":
            ...
            for multiplier, label in [(0.5, "Conservative"), (1.0, "Current"), (2.0, "Aggressive"), (3.0, "Transformative")]:
                ...
                prod_results = self.economic_models.calculate_productivity_gain(
                    ...
                    years=3,
                    industry=industry,
                    adoption_maturity=(scenario_investment / annual_revenue) * 20  # Rough adoption estimate
                )
            ...
            fig = make_subplots(
                rows=2, cols=2,
                subplot_titles=('ROI by Investment', 'NPV by Investment', 
            ...
            # Payback
            fig.add_trace(
                go.Bar(x=scenarios_df['Scenario'], y=scenarios_df['Payback (years)'], name='Payback'),
                row=2, col=1
            )
            
            # Productivity
            fig.add_trace(
                go.Bar(x=scenarios_df['Scenario'], y=scenarios_df['Productivity Gain %'], name='Productivity'),
                row=2, col=2
            )
            ...
            **Recommended Strategy ({risk_tolerance} Risk Profile):**
            ...
            for years, label in [(1, "Rapid (1 year)"), (2, "Fast (2 years)"), 
                               (3, "Moderate (3 years)"), (5, "Slow (5 years)")]:
    ...
    def _display_data_driven_action_plan(
        ...
        **Recommended Timeline:** {timeline}
        ...
        for phase_num, phase in enumerate(phases, 1):
            with st.expander(f"Phase {phase_num}: {phase['name']} ({phase['timeline']})", 
        ...
        if st.button("📥 Export Detailed Action Plan", use_container_width=True):
            plan_text = self._generate_exportable_plan(
                position, urgency, timeline, phases, 
                allocations, talent_needs, milestones,
                roi_projection, your_metrics, industry
            )
