...
class EconomicValidator:
    ...
    def validate_economic_inputs(
        ...
        if revenue is not None and employees is not None:
            cross_valid, cross_errors = self._cross_validate_revenue_employees(
                revenue, employees
            )
    ...
    def _validate_revenue(self, revenue: float) -> Tuple[bool, List[str], float]:
        ...
        if revenue <= 0:
            ...
            errors.append(f"Revenue exceeds maximum threshold (${self.constraints.MAX_REVENUE:,.0f})")
            confidence = 0.2
        else:
