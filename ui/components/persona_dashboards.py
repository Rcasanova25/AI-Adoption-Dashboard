...
class PersonaDashboards:
    ...
    def render_persona_selector(self) -> str:
        ...
        persona_icons = {"Executive": "👔", "Policymaker": "🏛️", "Researcher": "🔬", "General": "👥"}
    ...
    def _handle_quick_action(self, action: str):
        ...
        action_handlers = {
            ...
            "competitive_position": lambda: st.session_state.update(
                {"selected_view": "Competitive Position"}
            ),
            "geographic_view": lambda: st.session_state.update(
                {"selected_view": "Geographic Distribution"}
            ),
    ...
    def _render_executive_dashboard(self, data: Dict[str, pd.DataFrame]):
        ...
        with col1:
            self._render_metric_card(
                "AI Adoption", "87%", "+15% YoY", "Above industry average", color="green"
            )

        with col2:
            self._render_metric_card(
                "Competitive Position", "Leader", "Top 20%", "Ahead of 80% of peers", color="blue"
            )

        with col3:
            self._render_metric_card(
                "ROI Projection", "185%", "18 mo payback", "High confidence", color="green"
            )

        with col4:
            self._render_metric_card(
                "Investment Gap", "$2.5M", "To reach leaders", "Priority: High", color="yellow"
            )
    ...
    def _render_policymaker_dashboard(self, data: Dict[str, pd.DataFrame]):
        ...
        with col1:
            self._render_metric_card(
                "National Adoption", "72%", "+8% YoY", "OECD Average: 65%", color="green"
            )

        with col2:
            self._render_metric_card(
                "Labor Impact", "12% at risk", "45% augmented", "Net positive impact", color="blue"
            )

        with col3:
            self._render_metric_card(
                "GDP Impact", "+7.2%", "By 2030", "Above projections", color="green"
            )

        with col4:
            self._render_metric_card(
                "Policy Readiness", "65%", "Gaps identified", "Action needed", color="yellow"
            )
    ...
    def _render_researcher_dashboard(self, data: Dict[str, pd.DataFrame]):
        ...
        with col1:
            self._render_metric_card(
                "Data Points", "1.2M+", "28 sources", "95% validated", color="blue"
            )

        with col2:
            self._render_metric_card(
                "Time Series", "2017-2025", "8 years", "Monthly granularity", color="blue"
            )

        with col3:
            self._render_metric_card(
                "Coverage", "52 countries", "15 sectors", "Comprehensive", color="green"
            )

        with col4:
            self._render_metric_card(
                "Model Accuracy", "R² = 0.92", "p < 0.001", "Highly significant", color="green"
            )
    ...
    def _render_general_dashboard(self, data: Dict[str, pd.DataFrame]):
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["What is AI?", "Benefits", "Getting Started", "Success Stories"]
        )
    ...
    def _render_metric_card(
        self, title: str, value: str, delta: str, description: str, color: str = "blue"
    ):
        """Render a styled metric card."""
        color_map = {"blue": "#1f77b4", "green": "#28a745", "red": "#dc3545", "yellow": "#ffc107"}
    ...
    def _render_trend_analysis(self, data):
        ...
        if "adoption_rates" in data and not data["adoption_rates"].empty:
            ...
            st.line_chart(
                df.set_index("date")["adoption_rate"]
                if "date" in df.columns
                else df["adoption_rate"]
            )
        else:
...
def create_persona_specific_view(persona: str, view_type: str, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
