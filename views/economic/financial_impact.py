"""Financial Impact view module for AI Adoption Dashboard.

This module provides visualizations and analysis of the financial impact of AI adoption
by business function, including cost savings, revenue gains, and ROI insights.
"""
...
class FinancialImpactView(BaseView):
    def __init__(self):
        super().__init__(
            title="Financial Impact of AI by Business Function",
            description="Analysis of the financial impact of AI adoption by business function, including cost savings, revenue gains, and ROI insights."
        )
        self.a11y = AccessibilityManager()

    def render_content(self, data: Dict[str, pd.DataFrame]) -> None:
        """Render the Financial Impact view content.
        ...
        try:
            ...
            if financial_impact is None or financial_impact.empty:
                st.error(
                    "Required financial impact data is missing or empty. Please check data sources."
                )
                st.stop()
            ...
            fig = self.a11y.make_chart_accessible(
                ...
                title="Percentage of Companies Reporting Financial Benefits from AI",
                description=(
                    "Horizontal grouped bar chart showing percentage of companies reporting cost savings and revenue gains from AI by business function. "
                    "Marketing & Sales leads with 71% reporting revenue gains, while Service Operations shows 49% reporting cost savings. "
                    "Data shows the proportion of companies seeing benefits, with actual magnitude typically being modest (5-10% cost savings, 2-4% revenue gains)."
                ),
            )
            ...
            with st.expander("📊 Data Source & Methodology"):
                st.info(self._show_source_info("ai_index"))

        except Exception as e:
            st.error(f"Error rendering financial impact view: {str(e)}")

    def _show_source_info(self, source_key: str) -> str:
...
financial_impact_view = FinancialImpactView()

def render(data: Dict[str, Any]) -> None:
    financial_impact_view.render(data)