...
def run_linting():
    ...
    flake8_result = subprocess.run(
        ["flake8", "--max-line-length=100", "--exclude=tests"], capture_output=True
    )
...
def run_type_checking():
    ...
    mypy_result = subprocess.run(
        ["python", "-m", "mypy", "--ignore-missing-imports", "."], capture_output=True
    )
...
def main():
    ...
    test_result = run_tests(
        test_type=args.test_type, verbose=args.verbose, coverage=args.coverage or args.all_checks
    )
