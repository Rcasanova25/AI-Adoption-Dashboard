...
def load_data_enhanced() -> Tuple:
    ...
    if USE_OPTIMIZED_MANAGER and st.session_state.get("use_optimized_loader", True):
        try:
            ...
            ai_productivity_estimates = all_data.get(
                "productivity_estimates", legacy_load_data()[9]
            )
...
def inject_new_features():
    ...
    if HAS_COMPETITIVE_ASSESSOR and st.sidebar.button(
        "🏆 Competitive Assessment", key="comp_assess_btn"
    ):
    ...
    with st.container():
        ...
        with col2:
            ...
            if st.button("Get Assessment", type="primary", key="modal_assess"):
                assessment_data = {
                    "company": {"industry": industry, "size": company_size, "ai_maturity": ai_usage}
                }
                ...
                recommendations = assessor.generate_recommendations(
                    position, assessment_data["company"]
                )
    ...
    with st.container():
        ...
        with col1:
            current_position = st.slider(
                "Current AI Maturity (0-100)", 0, 100, 30, key="modal_maturity"
            )
        ...
        with col2:
            industry_avg = st.slider("Industry Average Maturity", 0, 100, 50, key="modal_ind_avg")

            years = st.slider("Time Horizon (years)", 1, 10, 5, key="modal_years")

        if st.button("Calculate Cost of Inaction", key="modal_calc"):
            cost = insights.calculate_cost_of_inaction(
                current_position, industry_avg, market_size, years
            )
