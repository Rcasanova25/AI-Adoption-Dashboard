...
from components.progressive_disclosure import DisclosureLevel, ProgressiveDisclosure
...
class TestProgressiveDisclosure:
    ...
    def test_render_with_disclosure_standard(self, disclosure, sample_content):
        ...
        with StreamlitTestHelper.mock_streamlit() as mocks:
            ...
            # Render content
            disclosure.render_with_disclosure(
                title="Test View", content=sample_content, current_level=DisclosureLevel.STANDARD
            )
    ...
    def test_render_with_disclosure_detailed(self, disclosure, sample_content):
        ...
        with StreamlitTestHelper.mock_streamlit() as mocks:
            # Render content
            disclosure.render_with_disclosure(
                title="Test View", content=sample_content, current_level=DisclosureLevel.DETAILED
            )
    ...
    def test_progressive_loading(self, disclosure):
        ...
        with StreamlitTestHelper.mock_streamlit() as mocks:
            ...
            content_shown = disclosure.render_progressive_content(
                sections=["Section 1", "Section 2", "Section 3", "Section 4"], initial_count=2
            )
            ...
            button_calls = [
                call for call in mocks["button"].call_args_list if "Show more" in str(call)
            ]
    ...
    def test_content_filtering(self, disclosure):
        ...
        exec_content = disclosure.filter_content(full_content, DisclosureLevel.EXECUTIVE)
        assert "basic_info" in exec_content
        assert "raw_data" not in exec_content

        # Test standard filtering
        std_content = disclosure.filter_content(full_content, DisclosureLevel.STANDARD)
        ...
        detailed_content = disclosure.filter_content(full_content, DisclosureLevel.DETAILED)
    ...
    def test_smart_summarization(self, disclosure):
        ...
        exec_summary = disclosure.summarize_for_level(
            detailed_text, DisclosureLevel.EXECUTIVE, max_sentences=3
        )

        std_summary = disclosure.summarize_for_level(
            detailed_text, DisclosureLevel.STANDARD, max_sentences=10
        )
    ...
    def test_visual_complexity_adjustment(self, disclosure):
        ...
        with StreamlitTestHelper.mock_streamlit() as mocks:
            ...
            for i in range(10):
                fig.add_trace(
                    go.Scatter(
                        x=list(range(100)), y=[j + i for j in range(100)], name=f"Series {i}"
                    )
                )

            # Simplify for executive
            exec_fig = disclosure.adjust_chart_complexity(fig, DisclosureLevel.EXECUTIVE)

            # Should have fewer traces
            assert len(exec_fig.data) < len(fig.data)

            # Detailed should keep all
            detailed_fig = disclosure.adjust_chart_complexity(fig, DisclosureLevel.DETAILED)
