...
class StreamlitTestHelper:
    ...
    def mock_streamlit():
        ...
        with patch("streamlit.write") as mock_write, patch(
            "streamlit.markdown"
        ) as mock_markdown, patch("streamlit.columns") as mock_columns, patch(
            "streamlit.metric"
        ) as mock_metric, patch(
            "streamlit.plotly_chart"
        ) as mock_plotly, patch(
            "streamlit.dataframe"
...
def create_test_dataframe(
    rows: int = 100, columns: List[str] = None, dtypes: Dict[str, type] = None
) -> pd.DataFrame:
...
def mock_pdf_extractor():
    ...
    mock.extract_tables.return_value = [
        pd.DataFrame({"Column1": ["A", "B", "C"], "Column2": [1, 2, 3]})
    ]
    return mock


def verify_data_structure(
    data: Any, expected_type: type, required_keys: Optional[List[str]] = None
):
...
def compare_dataframes(df1: pd.DataFrame, df2: pd.DataFrame, tolerance: float = 0.01) -> bool:
