...
class EnhancedPDFExtractor(PDFExtractor):
    ...
    def extract_tables(self, page_range: Optional[Tuple[int, int]] = None,
                      table_settings: Optional[Dict] = None) -> List[pd.DataFrame]:
    ...
    def find_table_by_keywords(self, keywords: List[str], 
                               context_lines: int = 5) -> Optional[pd.DataFrame]:
    ...
    def extract_data_by_pattern(self, pattern: str, 
                               page_range: Optional[Tuple[int, int]] = None) -> List[Dict[str, str]]:
    ...
    def extract_numeric_data(self, 
                           keywords: List[str],
                           value_pattern: str = r'([\d,]+\.?\d*)\s*(%|percent|million|billion|trillion)?') -> Dict[str, List[Tuple[float, str]]]:
        ...
        for keyword in keywords:
            ...
            keyword_pattern = re.compile(
                rf'{re.escape(keyword)}.{{0,100}}{value_pattern}',
                re.IGNORECASE
            )
