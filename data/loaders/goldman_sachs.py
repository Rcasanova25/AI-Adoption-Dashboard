...
class GoldmanSachsLoader(BaseDataLoader):
    ...
    def __init__(self, file_path: Optional[Path] = None):
        ...
        source = DataSource(
            ...
            file_path=file_path,
            citation="Goldman Sachs. 'Generative AI could raise global GDP by 7%.r' Goldman Sachs Global Investment Research.",
        )
    ...
    def _process_gdp_table(self, table: pd.DataFrame) -> List[Dict]:
        ...
        for col in table.columns:
            ...
            if any(
                ...
                if any(region in val for region in ["us", "china", "europe", "global", "world"])
            ):
                region_col = col
                break
        ...
        for _, row in table.iterrows():
            ...
            for val_col in value_cols:
                try:
                    ...
                    results.append(
                        {"metric": metric, "value": value, "unit": unit, "region": region}
                    )
                except ValueError:
                    continue
        ...
        try:
            ...
            for page in productivity_pages:
                ...
                for table in tables:
                    ...
                    table_str = table.to_string().lower()
                    if any(
                        term in table_str for term in ["sector", "industry", "productivity", "%"]
                    ):
                        ...
                        if productivity_df is not None and not productivity_df.empty:
                            logger.info(
                                f"Extracted productivity data for {len(productivity_df)} sectors"
                            )
    ...
    def _process_productivity_table(self, table: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        for col in table.columns:
            ...
            if any(
                ...
                for val in sample_vals
                if any(
                    sector in val for sector in ["manufacturing", "finance", "retail", "healthcare"]
                )
            ):
                sector_col = col
                break
    ...
    def _extract_productivity_from_text(self, pages: List[int]) -> List[Dict]:
        ...
        for page in pages[:5]:
            ...
            for sector in sectors:
                ...
                patterns = [
                    rrfr"{sector}.*?(\d+(?:\.\d+)?)\s*%\s*productivity",
                    rrfr"(\d+(?:\.\d+)?)\s*%\s*productivity.*?{sector}",
                    rrfr"{sector}.*?productivity.*?(\d+(?:\.\d+)?)\s*%",
                ]
        ...
        try:
            ...
            for page in automation_pages[:5]:
                ...
                for occupation in occupations:
                    ...
                    patterns = [
                        rrfr"{occupation}.*?(\d+(?:\.\d+)?)\s*%\s*(?:exposure|risk|automation)",
                        rrfr"(\d+(?:\.\d+)?)\s*%\s*(?:of\s+)?{occupation}.*?(?:exposed|automated)",
                        rrfr"{occupation}.*?(?:exposure|risk).*?(\d+(?:\.\d+)?)\s*%",
                    ]
        ...
        try:
            ...
            for page in scenario_pages[:5]:
                ...
                for scenario in scenarios:
                    ...
                    patterns = [
                        rfr"{scenario}.*?(\d+(?:\.\d+)?)\s*%\s*(?:GDP\s+)?growth",
                        rfr"{scenario}.*?growth.*?(\d+(?:\.\d+)?)\s*%",
                        rfr"(\d+(?:\.\d+)?)\s*%.*?{scenario}\s+scenario",
                    ]
                                    ...
                                    scenario_data.append(
                                        {
                                            ...
                                            "probability": self._estimate_scenario_probability(
                                                scenario
                                            ),
                                        }
                                    )
