...
class AIIndexLoader(BaseDataLoader):
    ...
    def _extract_adoption_trends(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            for table in tables:
                ...
                has_year = any('year' in str(col).lower() or 
                              bool(re.search(r'20\d{2}', str(col))) 
                              for col in table.columns)
                
                has_percentage = any('%' in str(col) or 
                                   'rate' in str(col).lower() or
                                   'adoption' in str(col).lower() 
                                   for col in table.columns)
        ...
        try:
            ...
            for page in sector_pages:
                ...
                for table in tables:
                    ...
                    sector_keywords = ['technology', 'financial', 'healthcare', 'manufacturing', 'retail']
    ...
    def _clean_sector_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if any('technology' in val or 'financial' in val or 'healthcare' in val 
        ...
        if sector_col and rate_cols:
            ...
            clean_df = clean_df[
                (clean_df['adoption_rate'] >= 0) & 
                (clean_df['adoption_rate'] <= 100)
            ]
        ...
        try:
            ...
            for page in geo_pages:
                ...
                for table in tables:
                    ...
                    table_str = table.to_string().lower()
                    if any(loc in table_str for loc in ['united states', 'china', 'europe', 'california', 'new york']):
    ...
    def _clean_geographic_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            sample_values = df[col].astype(str).str.lower()
            if any(loc in ' '.join(sample_values) for loc in ['state', 'country', 'city', 'united', 'china']):
                location_col = col
                break
    ...
    def _extract_firm_size_adoption(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['firm size', 'company size', 'employee', 'small business', 'enterprise', 'SMB']
            ...
            for page in size_pages:
                ...
                for table in tables:
                    ...
                    table_str = table.to_string().lower()
                    size_indicators = ['employee', '<50', '50-', '250-', '1000', '5000', 'small', 'medium', 'large']
    ...
    def _clean_firm_size_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if any(val for val in col_values if 
                   any(size in val for size in ['<', '>', '-', 'employee', 'small', 'medium', 'large'])):
                size_col = col
                break
    ...
    def _extract_ai_maturity(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['maturity', 'stage', 'phase', 'journey', 'adoption stage', 'implementation phase']
            ...
            for page in maturity_pages:
                ...
                for table in tables:
                    ...
                    table_str = table.to_string().lower()
                    maturity_keywords = ['exploring', 'experimenting', 'pilot', 'scaling', 'transform', 'mature']
    ...
    def _clean_maturity_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if any(val for val in col_values if 
                   any(stage in val for stage in ['explor', 'experiment', 'pilot', 'scal', 'transform'])):
                maturity_col = col
                break
        ...
        if maturity_col and pct_col:
            ...
            clean_df = clean_df.dropna()
            clean_df = clean_df[
                (clean_df['percentage_of_firms'] >= 0) & 
                (clean_df['percentage_of_firms'] <= 100)
            ]
    ...
    def _extract_investment_trends(self) -> Optional[pd.DataFrame]:
        ...
        try:
                        ...
                        try:
                            ...
                            if 2010 <= year <= 2030 and amount > 0:
                                investment_data.append({
                                    'year': year,
                                    'global_investment_billions': amount
                                })
                        except ValueError:
                            continue
            ...
            if investment_data:
                ...
                investment_df = investment_df.groupby('year').agg({
                    'global_investment_billions': 'max'  # Take max value for each year
                }).reset_index()
    ...
    def validate(self, data: Dict[str, pd.DataFrame]) -> bool:
        ...
        if 'adoption_trends' in data:
            ...
            if not any(col for col in trends.columns if 'adoption' in col.lower() or 'rate' in col.lower()):
    ...
    def _get_fallback_datasets(self) -> Dict[str, pd.DataFrame]:
        ...
        return {
            ...
            'firm_size_adoption': pd.DataFrame({
                'firm_size': ['<50', '50-249', '250+'],
                'adoption_rate': [14.8, 31.2, 65.0]
            }),
            ...
            'investment_trends': pd.DataFrame({
                'year': [2023, 2024, 2025],
                'global_investment_billions': [142.3, 189.5, 252.3]
            })
        }