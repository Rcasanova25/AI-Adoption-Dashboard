...
class OECDLoader(BaseDataLoader):
    ...
    def _extract_national_strategies(self) -> Optional[pd.DataFrame]:
        ...
        try:
            for extractor in self.extractors:
                ...
                keywords = ['national', 'strategy', 'country', 'government', 'policy', 
                ...
                countries = [
                    ...
                    'Denmark', 'Norway', 'Switzerland', 'Italy', 'Spain'
                ]
    ...
    def _extract_country_strategy(self, text: str, country: str) -> Optional[Dict]:
        ...
        # Extract launch year
        year_match = re.search(r'(20[1-2]\d)\s*(?:launch|establish|introduce|adopt)', context, re.IGNORECASE)
        if year_match:
        ...
        maturity_keywords = {
            'comprehensive': 2, 'advanced': 2, 'leading': 2,
            'established': 1.5, 'mature': 1.5, 'robust': 1.5,
            'developing': 1, 'emerging': 0.8, 'initial': 0.5
        }
        ...
        result['ai_governance_framework'] = any(term in context.lower() 
                                              for term in ['governance', 'regulatory', 'oversight'])
        
        # Check for ethics guidelines
        result['ethics_guidelines'] = any(term in context.lower() 
                                        for term in ['ethics', 'ethical', 'principles', 'responsible'])
    ...
    def _extract_policy_instruments(self) -> Optional[pd.DataFrame]:
        ...
        try:
            for extractor in self.extractors:
                ...
                keywords = ['policy', 'instrument', 'measure', 'initiative', 'program', 
                           'incentive', 'funding', 'regulatory']
                ...
                instruments = [
                    'R&D Funding', 'Tax Incentives', 'Regulatory Sandboxes',
                    ...
                    'Innovation Hubs', 'Testbeds', 'Certification Schemes'
                ]
    ...
    def _extract_principles_adoption(self) -> Optional[pd.DataFrame]:
        ...
        try:
            for extractor in self.extractors:
                ...
                keywords = ['principle', 'ethics', 'responsible', 'trustworthy', 
                           'transparent', 'accountable', 'fair', 'human']
    ...
    def _extract_regulatory_approaches(self) -> Optional[pd.DataFrame]:
        ...
        try:
            for extractor in self.extractors:
                ...
                keywords = ['regulatory', 'regulation', 'compliance', 'governance', 
                           'framework', 'approach', 'act', 'law']
    ...
    def _extract_regulatory_info(self, text: str, region: str) -> Optional[Dict]:
        ...
        alignment_match = re.search(r'industry\s*alignment\s*(?:score|rating)?\s*(?:of\s*)?(\d+(?:\.\d+)?)', 
                                  text, re.IGNORECASE)
        if alignment_match:
        ...
        # Compliance cost
        cost_match = re.search(r'compliance\s*cost\s*(?:index|score)?\s*(?:of\s*)?(\d+(?:\.\d+)?)', 
                             text, re.IGNORECASE)
        if cost_match:
    ...
    def _extract_international_cooperation(self) -> Optional[pd.DataFrame]:
        ...
        try:
            for extractor in self.extractors:
                ...
                keywords = ['international', 'cooperation', 'partnership', 'alliance', 
                           'multilateral', 'bilateral', 'global', 'initiative']
                ...
                initiatives = [
                    ...
                    'UNESCO AI Ethics',
                    'EU-US TTC', 'Trade and Technology Council',
                    'Quad AI',
    ...
    def _extract_initiative_info(self, text: str, initiative: str) -> Optional[Dict]:
        ...
        eff_match = re.search(r'effectiveness\s*(?:score|rating)?\s*(?:of\s*)?(\d+(?:\.\d+)?)', 
    ...
    def _extract_skills_initiatives(self) -> Optional[pd.DataFrame]:
        ...
        try:
            for extractor in self.extractors:
                ...
                keywords = ['skills', 'education', 'training', 'workforce', 'talent',
                           'reskilling', 'upskilling', 'curriculum']
                ...
                countries = [
                    'Singapore', 'Finland', 'Canada', 'South Korea', 'Germany',
                    'United Kingdom', 'France', 'Japan', 'Australia', 'Netherlands',
                    'Denmark', 'Sweden', 'United States', 'China', 'India'
                ]
    ...
    def _extract_country_skills(self, text: str, country: str) -> Optional[Dict]:
        ...
        result['ai_in_curriculum'] = any(term in context.lower() 
                                       for term in ['curriculum', 'education system', 'schools'])
        ...
        budget_match = re.search(r'\$(\d+(?:\.\d+)?)\s*(million|billion)\s*(?:for\s*)?training', 
                               context, re.IGNORECASE)
        if budget_match:
            amount = float(budget_match.group(1))
            if budget_match.group(2).lower() == 'billion':
    ...
    def _extract_public_investment(self) -> Optional[pd.DataFrame]:
        ...
        try:
            for extractor in self.extractors:
                ...
                keywords = ['investment', 'funding', 'budget', 'spending', 'expenditure',
                           'billion', 'million', 'public']
                ...
                for page in investment_pages[:5]:
                    ...
                    for table in tables:
                        ...
                        table_str = table.to_string().lower()
                        if ('20' in table_str and 
                            any(term in table_str for term in ['billion', 'investment', 'funding'])):
    ...
    def _get_fallback_datasets(self) -> Dict[str, pd.DataFrame]:
        ...
        return {
            ...
            'international_cooperation': pd.DataFrame({
                ...
                'focus_area': ['Responsible AI', 'Policy coordination', 'Ethics', 'Tech cooperation'],
