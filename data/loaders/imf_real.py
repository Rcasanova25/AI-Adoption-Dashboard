"""IMF AI economic analysis data loader with actual PDF extraction."""
...
class IMFLoader(BaseDataLoader):
    ...
    def _extract_macroeconomic_impact(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['GDP', 'macroeconomic', 'economic growth', 'productivity', 'inflation',
                       'unemployment', 'scenario', 'projection', 'forecast', 'baseline']
            ...
            scenarios = ['Baseline', 'Accelerated Adoption', 'Disruption', 'Stagnation', 
                        'Optimistic', 'Pessimistic', 'Conservative', 'Aggressive']
    ...
    def _process_macro_table(self, table: pd.DataFrame) -> List[Dict]:
        ...
        for col in table.columns:
            ...
            if any(val for val in sample_vals if 
                   any(scenario in val for scenario in ['baseline', 'optimistic', 'pessimistic'])):
                scenario_col = col
                break
    ...
    def _extract_fiscal_implications(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['fiscal', 'tax', 'budget', 'government', 'spending', 'revenue',
                       'public', 'social', 'UBI', 'universal basic income']
            ...
            impact_areas = [
                'Tax Revenue', 'Social Spending', 'Education Budget',
                'Infrastructure', 'R&D Subsidies', 'UBI Consideration',
                'Healthcare Spending', 'Unemployment Benefits', 'Pension Systems'
            ]
    ...
    def _extract_fiscal_area_data(self, text: str, area: str) -> Optional[Dict]:
        ...
        if any(term in context.lower() for term in ['ready', 'prepared', 'framework', 'established']):
            readiness_score += 2.0
        ...
        return result if 'revenue_impact_percent' in result or 'spending_change_percent' in result else None
    ...
    def _process_fiscal_table(self, table: pd.DataFrame) -> List[Dict]:
        ...
        for col in table.columns:
            sample_vals = table[col].astype(str).str.lower()
            if any(val for val in sample_vals if 
                   any(term in val for term in ['tax', 'spending', 'budget', 'revenue'])):
                area_col = col
                break
    ...
    def _extract_monetary_policy(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['monetary policy', 'central bank', 'interest rate', 'inflation targeting',
                       'financial stability', 'policy transmission', 'digital currency']
    ...
    def _extract_monetary_concern_data(self, text: str, concern: str) -> Optional[Dict]:
        ...
        if any(term in context.lower() for term in ['well-prepared', 'ready', 'equipped', 'capable']):
            preparedness_score += 2.5
        elif any(term in context.lower() for term in ['prepared', 'adequate', 'sufficient']):
            preparedness_score += 1.5
        elif any(term in context.lower() for term in ['unprepared', 'lacking', 'insufficient', 'limited']):
    ...
    def _extract_financial_stability(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['financial stability', 'systemic risk', 'algorithmic trading',
                       'credit risk', 'cyber risk', 'market concentration', 'regulatory']
            ...
            risk_categories = [
                'Algorithmic Trading', 'Credit Decisions', 'Cyber Risk',
                'Market Concentration', 'Systemic Bias', 'Regulatory Gaps',
                'Data Privacy', 'Model Risk', 'Operational Risk'
            ]
    ...
    def _extract_financial_risk_data(self, text: str, category: str) -> Optional[Dict]:
        ...
        for pattern in severity_patterns:
            ...
            severity_score = 5.0
            if any(term in context.lower() for term in ['high risk', 'severe', 'critical', 'extreme']):
                severity_score = 8.5
            elif any(term in context.lower() for term in ['moderate risk', 'significant', 'substantial']):
                severity_score = 7.0
    ...
    def _extract_emerging_markets(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['emerging markets', 'developing countries', 'low income', 'middle income',
                       'readiness', 'infrastructure', 'skills gap', 'digital divide']
            ...
            countries = [
                ...
                'Argentina', 'Colombia', 'Nigeria', 'Vietnam', 'Egypt'
            ]
    ...
    def _extract_country_readiness_data(self, text: str, country: str) -> Optional[Dict]:
        ...
        for pattern in readiness_patterns:
            ...
            readiness_score = 5.0
            if any(term in context.lower() for term in ['advanced', 'high readiness', 'well-prepared']):
                readiness_score = 7.5
        ...
        policy_score = 5.0
        if any(term in context.lower() for term in ['strong framework', 'comprehensive policy', 'robust']):
            policy_score = 7.5
        elif any(term in context.lower() for term in ['developing framework', 'improving', 'progress']):
            policy_score = 6.0
        elif any(term in context.lower() for term in ['weak framework', 'limited policy', 'lacking']):
            policy_score = 4.0
        ...
        if any(term in context.lower() for term in ['high potential', 'significant opportunity', 'promising']):
            growth_score = 8.5
        elif any(term in context.lower() for term in ['moderate potential', 'steady growth']):
            growth_score = 7.0
        elif any(term in context.lower() for term in ['limited potential', 'challenges', 'constraints']):
            growth_score = 5.5
    ...
    def _process_em_table(self, table: pd.DataFrame) -> List[Dict]:
        ...
        for col in table.columns:
            sample_vals = table[col].astype(str).str.lower()
            if any(val for val in sample_vals if 
                   any(country in val for country in ['india', 'brazil', 'mexico', 'indonesia'])):
                country_col = col
                break
    ...
    def _extract_trade_implications(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['trade', 'export', 'import', 'global value chain', 'supply chain',
                       'competitiveness', 'comparative advantage', 'trade policy']
    ...
    def _extract_trade_aspect_data(self, text: str, aspect: str) -> Optional[Dict]:
        ...
        if any(term in context.lower() for term in ['revolutionary', 'transformative', 'fundamental']):
            result['transformation_level'] = 'High'
        elif any(term in context.lower() for term in ['significant', 'substantial', 'considerable']):
        ...
        return result if 'impact_magnitude_percent' in result or 'transformation_level' in result else None
        ...
        return {
            ...
            'financial_stability': pd.DataFrame({
                'risk_category': ['Algorithmic Trading', 'Credit Decisions', 'Cyber Risk',
                                'Market Concentration', 'Systemic Bias'],
            ...
            'trade_implications': pd.DataFrame({
                'trade_aspect': ['Manufacturing Competitiveness', 'Services Trade', 
