...
class McKinseyLoader(BaseDataLoader):
    ...
    def __init__(self, file_path: Optional[Path] = None):
        ...
        source = DataSource(
            ...
            file_path=file_path,
            citation="McKinsey & Company. 'The State of AI in 2024.' McKinsey Global Institute."
        )
    ...
    def _extract_financial_impact(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['value creation', 'financial impact', 'cost reduction', 
                       'revenue increase', 'EBITDA', 'productivity', 'savings']
            ...
            for page in financial_pages[:10]:
                ...
                for table in tables:
                    ...
                    if any(term in table_str for term in ['cost', 'revenue', 'savings', 'increase', 'reduction', '%']):
    ...
    def _process_financial_table(self, table: pd.DataFrame) -> List[Dict]:
        ...
        for col in table.columns:
            ...
            if any(val for val in sample_vals if 
                   any(term in val for term in ['cost', 'revenue', 'productivity', 'savings', 'function'])):
                metric_col = col
                break
    ...
    def _extract_use_case_adoption(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['use case', 'application', 'deployment', 'function', 'department', 
                       'marketing', 'sales', 'supply chain', 'HR', 'finance', 'IT']
            ...
            for page in use_case_pages:
                ...
                for table in tables:
                    ...
                    table_str = table.to_string().lower()
                    function_keywords = ['marketing', 'sales', 'supply', 'operations', 'finance', 'hr', 'it']
                    ...
                    if sum(1 for kw in function_keywords if kw in table_str) >= 3:
                        ...
                        if use_case_df is not None and not use_case_df.empty:
                            logger.info(f"Extracted use case data with {len(use_case_df)} functions")
                            return use_case_df
    ...
    def _process_use_case_table(self, table: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        for col in table.columns:
            sample_vals = table[col].astype(str).str.lower()
            if any('marketing' in val or 'sales' in val or 'supply' in val or 'finance' in val 
    ...
    def _extract_use_cases_from_text(self, pages: List[int]) -> List[Dict]:
        ...
        use_case_data = []
        functions = ['Marketing', 'Sales', 'Supply Chain', 'Operations', 'Finance', 'HR', 'IT', 'R&D']
    ...
    def _extract_implementation_barriers(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['barrier', 'challenge', 'obstacle', 'difficulty', 'constraint', 
                       'limitation', 'risk', 'concern']
            ...
            for page in barrier_pages[:5]:
                ...
                for table in tables:
                    ...
                    table_str = table.to_string().lower()
                    if any(term in table_str for term in ['talent', 'data', 'integration', 'cost', 'security']):
    ...
    def _categorize_barrier(self, barrier: str) -> str:
        ...
        if any(term in barrier_lower for term in ['talent', 'skill', 'expertise', 'training']):
            ...
            return 'financial'
        elif any(term in barrier_lower for term in ['security', 'privacy', 'compliance', 'regulation']):
    ...
    def _extract_productivity_gains(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['productivity', 'efficiency', 'performance', 'output', 'automation', 'time savings']
    ...
    def _extract_risk_governance(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            keywords = ['risk', 'governance', 'compliance', 'ethics', 'responsible', 
    ...
    def _get_fallback_datasets(self) -> Dict[str, pd.DataFrame]:
        ...
        return {
            'financial_impact': pd.DataFrame({
                'metric': ['Cost reduction - Service Operations', 'Revenue increase - Marketing & Sales', 
            ...
            'use_case_adoption': pd.DataFrame({
                'function': ['Marketing & Sales', 'Service Operations', 'Supply Chain', 
                           'Software Engineering', 'Finance', 'HR'],
            ...
            'implementation_barriers': pd.DataFrame({
                'barrier': ['Lack of skilled talent', 'Data quality issues', 'Integration complexity',
                          'Unclear ROI', 'Security concerns'],
                'percentage': [68.0, 62.0, 58.0, 45.0, 48.0],
                'category': ['talent', 'data', 'technology', 'financial', 'security_compliance']
            }),
            'talent_metrics': pd.DataFrame({
                'metric': ['talent_gap_percentage', 'hiring_increase_percentage', 
            ...
            'risk_governance': pd.DataFrame({
                'governance_aspect': ['AI ethics guidelines', 'Risk assessment framework', 
                                    'Data privacy controls', 'Bias detection and mitigation'],
