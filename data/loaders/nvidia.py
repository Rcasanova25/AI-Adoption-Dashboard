...
class NVIDIATokenLoader(BaseDataLoader):
    ...
    def __init__(self, file_path: Optional[Path] = None):
        ...
        source = DataSource(
            ...
            file_path=file_path,
            citation="NVIDIA Corporation. 'Explaining Tokens — the Language and Currency of AI.r' NVIDIA Blog, 2024.",
        )
                                ...
                                optimization_data.append(
                                    {
                                        ...
                                        "metric_type": (
                                            "percentage" if "%" in pattern else "multiplier"
                                        ),
        ...
        try:
            ...
            for page in compute_pages[:5]:
                ...
                for table in tables:
                    ...
                    table_str = table.to_string().lower()
                    if any(
                        term in table_str for term in ["chatbot", "code", "translation", "analysis"]
                    ):
                        compute_df = self._process_compute_table(table)
                        if compute_df is not None and not compute_df.empty:
                            logger.info(
                                f"Extracted compute requirements for {len(compute_df)} use cases"
                            )
                            return compute_df
    ...
    def _process_compute_table(self, table: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        for col in table.columns:
            ...
            if any(
                ...
                if any(use in val for use in ["chat", "code", "content", "translation"])
            ):
                use_case_col = col
                break
